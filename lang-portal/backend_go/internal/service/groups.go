...
func (s *GroupsService) GetGroups(page int) ([]models.GroupWithStats, *models.Pagination, error) {
	itemsPerPage := 100
	...
	query := `
		SELECT 
			g.id,
			g.name,
			(SELECT COUNT(*) FROM words_groups WHERE group_id = g.id) as word_count
		FROM groups g
		ORDER BY g.name
		LIMIT ? OFFSET ?
	`
...
func (s *GroupsService) GetGroupWords(groupID, page int) ([]models.WordWithStats, *models.Pagination, error) {
	itemsPerPage := 100
	offset := (page - 1) * itemsPerPage
